...
data <- data.frame(day1[, c(3,4,5,8:13)])
...
linearMod <- lm(train_y[,1] ~ train_x[,1] + train_x[,2] + train_x[,3] + train_x[,4] + train_x[,5] + train_x[,6] + train_x[,7] + train_x[,8])
summary(linearMod)
cfs <- coef(linearMod)
cat("The linearModel which predict windspeed has weight ", cfs, "\n", sep="")
...
pred_y <- round(pred_y)
y_test <- as.num':eric(as.character(unlist(test_y[[1]])))
